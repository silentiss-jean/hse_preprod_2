...
export function getAbonnementHT(userData, periodIdx) {
    const v = Number(userData.abonnement_ht ?? userData.abonnementht ?? 0);
...
export function getAbonnementTTC(userData, periodIdx) {
    const v = Number(userData.abonnement_ttc ?? userData.abonnementttc ?? 0);
    const { abDiv } = PERIODS[periodIdx];
...
export function getTarifs(userData) {
    const type_contrat = userData.type_contrat || 'fixe';
    
    if (type_contrat === 'fixe') {
        return {
